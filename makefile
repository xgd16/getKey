...
.PHONY: linux-amd64
linux-amd64:
	CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build $(RACE) -o ./bin/${APP}_linux_amd64 ./main.go
.PHONY: linux-arm64
linux-arm64:
	CGO_ENABLED=0 GOOS=linux GOARCH=arm64 go build $(RACE) -o ./bin/${APP}_linux_arm64 ./main.go

## win: 编译打包win
.PHONY: win-amd64
win-amd64:
	CGO_ENABLED=0 GOOS=windows GOARCH=amd64 go build $(RACE) -o ./bin/${APP}_windows_amd64.exe ./main.go
.PHONY: win-arm64
win-arm64:
	CGO_ENABLED=0 GOOS=windows GOARCH=arm64 go build $(RACE) -o ./bin/${APP}_windows_arm64.exe ./main.go

## mac: 编译打包mac
.PHONY: mac-amd64
mac-amd64:
	CGO_ENABLED=0 GOOS=darwin GOARCH=amd64 go build $(RACE) -o ./bin/${APP}_darwin_amd64 ./main.go
.PHONY: mac-arm64
mac-arm64:
	CGO_ENABLED=0 GOOS=darwin GOARCH=arm64 go build $(RACE) -o ./bin/${APP}_darwin_arm64 ./main.go
